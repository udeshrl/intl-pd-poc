...
public class QuizResource {
	...
	public ListUserType getQuizzes() {
		...
		for (PDClass teacher : tearcherclass) {
			...
			for (User studentUser : teacher.getStudentUser()) {
				...
				System.out.println("Student User FristName "
						+ studentUser.getFirst_name());
				System.out.println("Student User FristName "
						+ studentUser.getLast_name());
				System.out.println("Student User FristName "
						+ studentUser.getRole());
	...
	@Produces(MediaType.APPLICATION_JSON)
	public ListQuizeType getQuizzesData(
		...
		List<StudentQuizActivity> studentActity = quizService
				.getStudentQuizActitiy(studentId);
		for (StudentQuizActivity activity : studentActity) {
			...
			for (QuizeQuestion question : activity.getQuize().getQuestions()) {
				...
				for (Book book : question.getBookQuestion()) {
					BookType booktype = new BookType();
					booktype.setId(book.getId());
					booktype.setName(book.getName());
					questionsType.addbookType(booktype);
				}
				for (Chapter chapter : question.getChapterQuestion()) {
					...
					questionsType.addchapterType(chapterType);
				}
				for (Concept concept : question.getConceptQuestion()) {
					ConceptType conceptType = new ConceptType();
					conceptType.setId(concept.getId());
					conceptType.setName(concept.getName());
					questionsType.addconceptType(conceptType);
				}
				for (Topic topic : question.getTopicQuestion()) {
					TopicType topicType = new TopicType();
					topicType.setId(topic.getId());
					topicType.setName(topic.getName());
					topicType.setCode(topic.getCode());
					questionsType.addtopicType(topicType);
				}
			}
	...
	public ListQuizeResultType getQuizzeResult(
		...
		for (StudentQuizActivity activity : studentActity) {
			...
			quizeResultType.setResultQuestionObject(activity
					.getResultQuestionObject());
			quizeResultType.setTotalQuestions(activity
					.getTotalNumberQuestionIssued());
			quizeResultType.setCorrectAnswers(activity
					.getNumQuestionAnsweredCorrect());
